...
platform <- "cosmx"
hESCs <- readRDS(paste0("/seurat/", sample, "_", platform, ".rds"))@assays$SCT@data
pred.hesc <- SingleR(test = hESCs, ref = hpca.se, assay.type.test=1,
                     labels = hpca.se$label)
saveRDS(pred.hesc, paste0("data/output/SingleR/predictions_", sample, "_", platform, "_whole_final.rds"))
