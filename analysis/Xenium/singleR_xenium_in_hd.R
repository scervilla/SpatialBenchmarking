...
hESCs <- readRDS(paste0(main_path,"/seurat_aligned/", sample, "_xenium_in_hd.rds"))@assays$SCT@data
pred.hesc <- SingleR(test = hESCs, ref = hpca.se, assay.type.test=1,
                     labels = hpca.se$label)
saveRDS(pred.hesc, paste0("data/output/SingleR/predictions_", sample, "_xenium_in_hd_final.rds"))
