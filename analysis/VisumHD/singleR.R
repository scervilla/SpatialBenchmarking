...
hESCs <- readRDS(paste0(main_path,"/seurat/", sample, "_HD_sct.rds"))@assays$SCT@data
pred.hesc <- SingleR(test = hESCs, ref = hpca.se, assay.type.test=1,
                     labels = hpca.se$label)
saveRDS(pred.hesc, paste0("data/output/SingleR/predictions_", sample, "_VisiumHD_final.rds"))
