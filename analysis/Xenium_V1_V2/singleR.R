...
hESCs <- readRDS(paste0(main_path, "/seurat_aligned/",  sample, "_aligned_sct_exp.rds"))
pred.hesc <- SingleR(test = hESCs, ref = hpca.se, assay.type.test=1,
                     labels = hpca.se$label)

saveRDS(pred.hesc, paste0("data/output/SingleR/predictions_", sample, "_aligned_final.rds"))
